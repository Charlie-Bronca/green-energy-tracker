...
function Scotland() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://api.carbonintensity.org.uk/regional/scotland');
  ...
  const createChart = (data) => {
    const ctx = document.getElementById('scotlandChart').getContext('2d');
  ...
  return (
    <div>
      <h1 style={{ fontSize: '20px' }}>Scotland</h1>
      <canvas id="scotlandChart" width="250" height="250" alt={altState}></canvas>
    </div>
  );
}
